...
const startButton = document.querySelector('.start-button'),
    ...
    firstFiledset = document.querySelector('.first-filedset');
...
const dopOptionsString = (yandex, google, order) => {
    //Подключаем Яндекс Метрику, Гугл Аналитику и отправку запросов на почту.
    ...
    if (yandex || google || order) {
        ...
        if (yandex) {
            ...
            if (google && order) {
                str += ', Гугл Аналитику и отправку запросов на почту.';
                return str;
            }
        ...
        if (order) {
            str += ' отправку запросов на почту'
        }
        str += '.';
    }
...
const moveBackTotal = () => {
    if (document.documentElement.getBoundingClientRect().bottom > document.documentElement.clientHeight + 200) {
        ...
        //firstFiledset.after(totalPrice);
